...
export function PageFrame({ children, eyebrow, title, intro }: { children: React.ReactNode; eyebrow: string; title: React.ReactNode; intro?: string }) {
  return <main className="site-shell inner-page"><div className="grain" aria-hidden="true" /><div className="signal-rail" aria-hidden="true"><span className="rail-mark">FC</span><span className="rail-line" /><span className="rail-vertical">FOXY CODENAME / 2026</span><span className="rail-index">001—003</span></div><SiteHeader /><section className="page-heading"><p className="eyebrow"><Radio size={14} /> {eyebrow}</p><h1>{title}</h1>{intro && <p>{intro}</p>}</section>{children}</main>;
}
